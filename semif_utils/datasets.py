...
class Pot:
    ...
    def save_config(self):
        try:
            save_pot_path = Path(self.data_home, self.pot_path)
            with open(save_pot_path, "w") as f:
                json.dump(self.config, f, indent=4, default=str)
        except Exception as e:
            raise e
        return True

    def save_pot(self, cutout_array):
...
class Background:
    ...
    def array(self):
        # Read the image from the file and return the numpy array
        back_array = cv2.imread(self.background_path, cv2.IMREAD_UNCHANGED)
        back_array = np.ascontiguousarray(
            cv2.cvtColor(back_array, cv2.COLOR_BGR2RGBA))
        return back_array
    ...
    def save_config(self):
        try:
            save_back_path = Path(self.data_home, self.background_path)
            with open(save_back_path, "w") as f:
                json.dump(self.config, f, indent=4, default=str)
        except Exception as e:
            raise e
        return True
